...
extern "C" void enc28j60_soft_reset(); //#include "enc28j60.h"
extern uint8_t TCP_NO_TRANS;
extern uint8_t TCP_NO_TRANS_MAX;
...
LinxListener::LinxListener()
{
    ...
    this->AttachCustomCommand(CUSTOM_CMD_CONTROL - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_CONTROL_f);
    this->AttachCustomCommand(CUSTOM_CMD_STATUS - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_STATUS_f);
    this->AttachCustomCommand(CUSTOM_CMD_SET_PARAMS - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_SET_PARAMS_f);
    ...
    this->AttachCustomCommand(CUSTOM_CMD_SET_U - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_SET_U_f);
    this->AttachCustomCommand(CUSTOM_CMD_SET_I - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_SET_I_f);
    this->AttachCustomCommand(CUSTOM_CMD_TEST - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_TEST_f);
    this->AttachCustomCommand(CUSTOM_CMD_DEBUG - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_DEBUG_f);
    this->AttachCustomCommand(CUSTOM_CMD_SET_PWM - CMD_OFFSET, &LinxCustomCmdHandler::CUSTOM_CMD_SET_PWM_f);
}
...
int LinxListener::ProcessCommand(unsigned char* commandPacketBuffer, unsigned char* responsePacketBuffer)
{
	...
	****************************************************************************************/	
	if(command >= 0xFC00)	
	{
		...
		if ((command >= CMD_OFFSET) && (command <= (CMD_OFFSET + 0x0A))){ //
...
void LinxListener::PacketizeAndSend(unsigned char* commandPacketBuffer, unsigned char* responsePacketBuffer, unsigned int dataSize,  int status)
{
	...
	if (PACKET_NUM == ((commandPacketBuffer[2] << 8) | commandPacketBuffer[3])) {
		//resend of packet? then
		//enc28j60_soft_reset();
	}
	PACKET_NUM = ((commandPacketBuffer[2] << 8) | commandPacketBuffer[3]);
